...
const MindMap = forwardRef<MindMapActions, MindMapProps>((props, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    zoomIn: () => {
      if (zoomBehaviorRef.current && svgRef.current) {
        ...
        svg.transition('zoom').duration(250).call(zoomBehavior.scaleBy, 1.2);
      }
    },
    zoomOut: () => {
      if (zoomBehaviorRef.current && svgRef.current) {
        const svg = select(svgRef.current);
        const zoomBehavior = zoomBehaviorRef.current;
        svg.transition('zoom').duration(250).call(zoomBehavior.scaleBy, 0.8);
      }
    },
    zoomToFit: () => {
      ...
      // Use direct invocation instead of .call for better type compatibility in some setups
      svg.transition('zoom').duration(400).call(zoomBehavior.transform, newTransform);
    }
  }));
  ...
  useEffect(() => {
    ...
    zoomBehavior
      ...
      .filter((evt) => {
        ...
        return toolMode === 'pan' && target.closest('.node-group, .editable-link-group, .node-toolbar-wrapper, .focus-bar') === null;
      })
      .on('zoom', ({ transform }) => {
  ...
  useEffect(() => {
    ...
    const selectionDrag = drag<SVGSVGElement, unknown>()
        ...
        .on('start', (event, _d) => {
            ...
            const [x, y] = pointer(event, gRef.current!);
            startPos = { x, y };
        ...
        .on('drag', (event, _d) => {
            if (!startPos) return;
            const [cursorX, cursorY] = pointer(event, gRef.current!);
  ...
  useEffect(() => {
    ...
    svg.transition('focus').duration(400).ease(easeCubicOut).call(zoomBehaviorRef.current.transform, newTransform);
  ...
  }, [nodes, d3Root, selectedNodeIds, onNodeSelect, onNodeDragStart, onMultipleNodePositionsUpdate, onNodeMove, isDescendant, focusedNodeId, root.id]);
