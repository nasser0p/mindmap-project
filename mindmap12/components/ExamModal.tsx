            ...
            <div className="mb-6">
                ...
                <div className="grid grid-cols-3 gap-3">
                    {(['Quiz', 'Midterm', 'Final'] as const).map((type, i) => (
                        <button key={type} onClick={() => handleTypeChange(type, (i+1)*5)} className={`text-center p-3 rounded-lg border transition-all ${config.type === type ? 'bg-blue-50 dark:bg-blue-950 border-blue-500 ring-2 ring-blue-200' : 'bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 border-slate-300 dark:border-slate-600'}`}>
...
const LoadingView: React.FC = () => (
    <div className="text-center text-white">
        ...
        <h2 className="text-2xl font-bold mt-4 animate-pulse">Generating your exam...</h2>
        <p className="mt-2">The AI is analyzing your notes to create the perfect test.</p>
    </div>
);
...
const ExamActiveView: React.FC<{ questions: Question[], onSubmit: (answers: Map<string, string>, revealedHints: Set<string>) => void }> = ({ questions, onSubmit }) => {
    ...
    return (
        <div className="w-full max-w-3xl h-full flex flex-col bg-white dark:bg-slate-800 rounded-xl shadow-2xl p-8 overflow-hidden">
            <div className="mb-4">
                    ...
                    >
                        ...
                        {isHintRevealed && (
                            <motion.div
                                ...
                                transition={{ duration: 0.3 }}
                                className="bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 p-3 rounded-lg text-sm text-yellow-800 dark:text-yellow-200 mb-6"
                            >
                            ...
                            <div className="space-y-3">
                                {currentQuestion.options?.map((option, index) => (
                                    <button
                                        ...
                                        className={`w-full text-left p-4 rounded-lg border-2 transition-all ${answers.get(currentQuestion.id) === option ? 'bg-blue-50 dark:bg-blue-950/80 border-blue-500' : 'bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 border-slate-200 dark:border-slate-600'}`}
                                    >
                        ...
                        {(currentQuestion.type === 'short-answer' || currentQuestion.type === 'fill-in-the-blank') && (
                            <textarea
                                ...
                                className="w-full h-40 p-3 border-2 border-slate-300 dark:border-slate-600 dark:bg-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
                            />
                        )}
                    </motion.div>
                </AnimatePresence>
            ...
            <div className="mt-auto pt-6 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center">
                <button onClick={handlePrev} disabled={currentIndex === 0} className="px-6 py-2 bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-slate-200 font-semibold rounded-lg hover:bg-slate-300 dark:hover:bg-slate-500 disabled:opacity-50 disabled:cursor-not-allowed">
                    Previous
                </button>
...
const ResultsView: React.FC<{ results: ExamResult, onClose: () => void }> = ({ results, onClose }) => {
    ...
    return (
        <div className="w-full max-w-3xl h-[90vh] flex flex-col bg-white dark:bg-slate-800 rounded-xl shadow-2xl overflow-hidden">
            <div className="p-6 text-center border-b border-slate-200 dark:border-slate-700">
                <h2 className="text-3xl font-bold text-slate-800 dark:text-slate-100">Exam Results</h2>
                <p className={`text-6xl font-bold mt-4 mb-2 ${scoreColorClass}`}>{results.score}%</p>
                <p className="text-slate-500 dark:text-slate-400">You got {results.analysis.length - incorrectAnswers.length} out of {results.analysis.length} correct.</p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50/50 dark:bg-slate-900/50">
                ...
                {incorrectAnswers.length > 0 ? (
                    incorrectAnswers.map((answer, index) => (
                        <div key={index} className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-red-200 dark:border-red-900/50">
...
const ExamModal: React.FC<ExamModalProps> = ({ state, branchExamConfig, onStart, onSubmit, onClose }) => {
    ...
    return (
        <AnimatePresence>
            ...
            >
                <motion.div
                    className={`${isExamActive ? 'w-full h-full' : 'max-w-lg'} relative`}
                    initial="hidden"
                    animate="visible"
                    exit="hidden"
