  ...
  useEffect(() => {
    ...
    const zoomBehavior = zoom<SVGSVGElement, unknown>()
      ...
      .filter((evt) => {
        ...
        return toolMode === 'pan' && (evt.target as Element).closest('.node-group, .editable-link-group, .node-toolbar-wrapper, .focus-bar, .topic-hotspot-wrapper, .node-context-menu') === null;
      })
      .on('zoom', (event) => {
  ...
  return (
    <div className="w-full h-full relative overflow-hidden dotted-background">
      <svg
        ...
        onContextMenu={(e) => { if(e.target === svgRef.current) e.preventDefault(); }} // Prevent browser context menu on background
        tabIndex={0}
