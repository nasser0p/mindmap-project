...
const Node: React.FC<NodeProps> = (props) => {
  const { 
    d3Node, nodeData, depth, x, y, isSelected, isBeingDragged, isDropTarget, isGlowing, startInEditMode,
  ...
  const strokeWidth = isSelected || isDropTarget || isGlowing ? 2.5 : 2;
          ...
          <motion.div className="w-full h-full rounded-3xl absolute top-0 left-0" style={{ border: `${strokeWidth}px solid ${strokeColor}`, pointerEvents: 'none' }}
             animate={{ 
                ...
                boxShadow: isDropTarget 
                    ? `0 0 12px 2px ${animatedStrokeColor}99` 
                    : (isGlowing ? `0 0 16px 3px ${color}99` : 'none')
             }}
             transition={{ 
                borderColor: { duration: 0.2 }, 
                boxShadow: (isDropTarget || isGlowing) 
