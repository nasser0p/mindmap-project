...
const Node: React.FC<NodeProps> = (props) => {
  const { 
    ...
    originalChildrenCount, theme, onSelect, onUpdate, onDelete, onToggleCollapse, onEditComplete,
  ...
  const handleTextClick = (e: React.MouseEvent) => {
    ...
    if (!isSelected) {
        return; // First click just selects.
    }
    
    const textEl = textRef.current;
    if (!textEl) return;
    
    // Use modern API to find character index from click coordinates
    let offset = 0;
    ...
    if (e.detail === 1) { // Second single click
        enterEditMode({ selectionStart: offset, selectionEnd: offset });
    } else if (e.detail === 2) { // Double click
        const { start, end } = findWordBoundaries(text, offset);
        enterEditMode({ selectionStart: start, selectionEnd: end });
    } else if (e.detail >= 3) { // Triple click
        ...
        {isEditing ? (
          <>
            ...
            <textarea 
                ...
                className="w-full bg-transparent text-slate-800 dark:text-slate-100 text-center font-medium focus:outline-none resize-none overflow-hidden" 
                style={{ minHeight: `${minNodeHeight - 26}px` }} 
                rows={1} 
            />
          </>
        ) : (
