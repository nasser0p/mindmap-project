...
const AttachmentsHub: React.FC<{ selectedNodes: MindMapNode[], onAddAttachment: (nodeId: string, attachmentData: Omit<Attachment, 'id'>, file?: File) => void; onUpdateAttachment: (nodeId: string, attachmentId: string, updatedContent: Attachment['content']) => void; onDeleteAttachment: (nodeId: string, attachmentId: string) => void; }> = ({ selectedNodes, onAddAttachment, onUpdateAttachment, onDeleteAttachment }) => {
    ...
    return (
        <div className="flex-1 flex flex-col h-full">
            ...
            {!editingAttachment && (
                <div className="pt-4 mt-auto border-t border-slate-200/80 dark:border-slate-700/80 flex items-center justify-around flex-shrink-0">
    ...
    return (
        <div className="flex-1 flex flex-col h-full">
            <div className="flex-shrink-0 mb-3 flex justify-between items-center">
                ...
                <button onClick={handleUploadClick} data-tutorial-id="upload-file-button" className="px-3 py-1 text-xs font-semibold bg-blue-100 text-blue-700 rounded-full hover:bg-blue-200 transition-colors">
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                {file.status === 'ready' && (
                                    <>
                                        ...
                                        <button onClick={() => onGenerateNodes(file)} data-tutorial-id="generate-nodes-button" disabled={generatingNodesFromFileId === file.id} title="Generate nodes from this document" className="w-6 h-6 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center justify-center text-blue-500 disabled:opacity-50">
...
const AiAssistant: React.FC<AiAssistantProps> = (props) => {
    const { 
        isOpen, onOpen, onClose, isMobile, selectedNodes, activeTab, onTabChange,
    ...
    return (
        <motion.div
            layoutId="ai-assistant-bubble"
            className="fixed z-30 w-full bottom-0 right-0 rounded-t-2xl md:w-[420px] md:bottom-6 md:right-6 md:rounded-2xl bg-slate-100/80 dark:bg-slate-900/80 backdrop-blur-lg shadow-2xl flex flex-col border border-transparent dark:border-slate-700"
            transition={transition}
            style={{ 
        ...
        >
            ...
            <div className="flex-1 flex flex-col p-4 overflow-hidden">
                ...
                {selectedNodes.length > 0 ? (
                    <div className="flex-1 flex flex-col min-h-0">
                        <div className="flex-shrink-0 bg-slate-100/80 dark:bg-transparent">
                            ...
                            <div className="flex border-b border-slate-300/60 dark:border-slate-700/60">
                                ...
                                <TabButton text="Documents" data-tutorial-id="documents-tab" icon="fa-file-lines" isActive={activeTab === 'documents'} onClick={() => onTabChange('documents')} count={documentCount} />
                            </div>
                        </div>
