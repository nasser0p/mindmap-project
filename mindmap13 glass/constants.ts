...
/**
 * Converts an RGB color array to an rgba string.
 * @param rgb The RGB color array [R, G, B].
 * @param alpha The alpha transparency value (0-1).
 * @returns The rgba color string.
 */
const rgbToRgba = (rgb: [number, number, number], alpha: number): string => {
    return `rgba(${rgb[0]}, ${rgb[1]}, ${rgb[2]}, ${alpha})`;
};

/**
 * Calculates a background color for a node based on its mastery score for the glassmorphism effect.
 * It creates a subtle tint from red to yellow to green on a semi-transparent base.
 * @param masteryScore The mastery score from 0 to 1.
 * @param theme The current theme ('light' or 'dark').
 * @returns An rgba color string.
 */
export const getMasteryBackgroundColor = (masteryScore: number, theme: 'light' | 'dark'): string => {
    const LIGHT_BASE_RGB: [number, number, number] = [255, 255, 255]; // white
    const DARK_BASE_RGB: [number, number, number] = [51, 65, 85];     // slate-700
    const GLASS_ALPHA = 0.6;
    const TINT_WEIGHT = 0.3; // 30% mastery color, 70% base color

    if (masteryScore <= 0) {
        return theme === 'light' ? `rgba(255, 255, 255, ${GLASS_ALPHA})` : `rgba(51, 65, 85, ${GLASS_ALPHA})`;
    }
    ...
    const baseColor = theme === 'light' ? LIGHT_BASE_RGB : DARK_BASE_RGB;
    const finalRgb = blendRgb(masteryColor, baseColor, TINT_WEIGHT);

    return rgbToRgba(finalRgb, GLASS_ALPHA);
};