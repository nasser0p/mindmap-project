...
const createNewChapterObject = (name: string, color: string, order: number, ownerId: string): Omit<Chapter, 'id'> => {
    const rootId = uuidv4();
    return {
        name,
        order,
...
const useMindMapData = (userId: string | null) => {
  ...
  const addSubject = useCallback(async () => {
    ...
    try {
        ...
        const firstChapterData = createNewChapterObject('Main Chapter', newSubjectColor, 0, userId);
  ...
  const addChapter = useCallback(async (name: string) => {
    ...
    const newChapterData = createNewChapterObject(name, color, order, userId);
    await db.collection('documents').doc(activeSubjectId).collection('chapters').add(newChapterData);
  }, [userId, activeSubjectId, chapters]);
  ...
  const addChildNode = useCallback((parentId: string, text: string): string | null => {
    if (!activeChapter) return null;
    const parentNode = findNode(parentId);
    if (!parentNode || parentNode.x === undefined || parentNode.y === undefined) return null;
  ...
  const addMultipleChildrenNode = useCallback((parentId: string, ideas: { text: string; note?: string }[]) => {
  ...
  const updateNodeSize = useCallback(async (nodeId: string, width: number, height: number) => {
    if (!activeChapterRef || !activeChapter) return;
    const newRoot = mapNodeRecursive(nodeId, activeChapter.root, (node) => ({ ...node, width, height }));
  ...
  const moveNode = useCallback((sourceId: string, targetId: string) => {
    ...
    const removeRecursive = (node: MindMapNode, id: string): MindMapNode | null => {
      if (node.id === id) {
        movingNode = { ...node };
      ...
      let childrenChanged = false;
      const newChildren = node.children.map(child => {
        const newChild = removeRecursive(child, id);
        if (newChild !== child) {
          childrenChanged = true;
        }
        return newChild;
    ...
    if (!movingNode || !treeAfterRemoval) return;
  ...
  const setNodeImage = useCallback((nodeId: string, imageInfo: { downloadURL: string; storagePath: string; mimeType: string; } | null) => {
    if (!activeChapter) return;
  ...
  const insertNodeBetween = useCallback((parentId: string, childId: string): string | null => {
    ...
    const newNode: MindMapNode = {
      id: uuidv4(),
      text: 'New Intermediate Idea',
      x: (parentNode.x + childNode.x) / 2,
      y: (parentNode.y + childNode.y) / 2,
      children: [childNode],
  ...
  return {
    ...
    addSourceDocument: (doc: SourceDocumentFile) => {
        if (!activeSubject) return;
        const newDocs = [...activeSubject.sourceDocuments, doc];
        updateSourceDocuments(newDocs);
    },
    updateSourceDocument: (docId: string, updates: Partial<SourceDocumentFile>) => {
        if (!activeSubject) return;
        const newDocs = activeSubject.sourceDocuments.map(d => d.id === docId ? {...d, ...updates} : d);
        updateSourceDocuments(newDocs);
    },
    deleteSourceDocument: (docId: string) => {
        if (!activeSubject) return;
        const newDocs = activeSubject.sourceDocuments.filter(d => d.id !== docId);
        updateSourceDocuments(newDocs);
    },
    addAttachment: () => {},
    updateAttachment: () => {},
    ...
    addLink: (sourceId: string, targetId: string) => {
        if(!activeChapter) return;
        const newLink: MindMapLink = { id: uuidv4(), source: sourceId, target: targetId, label: 'Relates to' };
        updateMindMap({ links: [...activeChapter.links, newLink]});
    },
    updateLinkLabel: (linkId: string, label: string) => {
        if(!activeChapter) return;
        const newLinks = activeChapter.links.map(l => l.id === linkId ? {...l, label} : l);
        updateMindMap({ links: newLinks });
    },
    deleteLink: (linkId: string) => {
        if(!activeChapter) return;
        const newLinks = activeChapter.links.filter(l => l.id !== linkId);
        updateMindMap({ links: newLinks });
    },
