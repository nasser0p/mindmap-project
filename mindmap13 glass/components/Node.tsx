...
const Node: React.FC<NodeProps> = (props) => {
  const { 
    d3Node, nodeData, depth, x, y, transform, isSelected, isBeingDragged, isPastingImage, glowSeverity, 
  ...
  const wasGestureHandled = useRef(false);
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
      wasGestureHandled.current = false;
      // Only handle single touches for long press, multi-touch is for pinch-zoom
      if (e.touches.length === 1) {
          const touch = e.touches[0];
          touchStartPosRef.current = { clientX: touch.clientX, clientY: touch.clientY };

          longPressTimerRef.current = setTimeout(() => {
              wasGestureHandled.current = true; // Mark gesture as handled
              onShowContextMenu(id, touch.clientX, touch.clientY);
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
      if (longPressTimerRef.current && touchStartPosRef.current) {
          ...
          if (distance > TOUCH_MOVE_THRESHOLD) {
              ...
              longPressTimerRef.current = null;
              touchStartPosRef.current = null;
              wasGestureHandled.current = true; // It's a drag
          }
      }
  };
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
      ...
      // If a long press didn't happen, and it wasn't a drag, it's a tap for selection
      if (!wasGestureHandled.current) {
          onSelect(id, e);
      }
  };
  ...
  const animatedStrokeColor = strokeColor;
  ...
  const finalClassName = `node-group absolute mind-map-node-draggable-part rounded-3xl flex flex-col items-center justify-start backdrop-blur-md`;

  const bgColor = isRoot
    ? (theme === 'light' ? 'rgba(239, 246, 255, 0.5)' : 'rgba(30, 41, 59, 0.5)') // Special glass for root
    : getMasteryBackgroundColor(masteryScore, theme);
