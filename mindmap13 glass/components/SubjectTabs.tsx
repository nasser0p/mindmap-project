...
const SubjectTab: React.FC<{
  ...
  const activeClasses = 'bg-slate-100/80 dark:bg-slate-900/80';
...
const SubjectTabs: React.FC<SubjectTabsProps> = (props) => {
  ...
  return (
    <header className="relative z-30 bg-white/70 dark:bg-slate-800/70 backdrop-blur-md border-b border-white/20 dark:border-slate-700/50 px-4 pt-2 flex items-center justify-between gap-4 flex-shrink-0">
      <div className="flex-1 min-w-0 relative">
        ...
        {/* Left Fade */}
        <div className={`absolute top-0 left-0 bottom-0 w-8 bg-gradient-to-r from-white/70 dark:from-slate-800/70 to-transparent pointer-events-none transition-opacity duration-300 ${showLeftFade ? 'opacity-100' : 'opacity-0'}`} aria-hidden="true" />
        {/* Right Fade */}
        <div className={`absolute top-0 right-0 bottom-0 w-8 bg-gradient-to-l from-white/70 dark:from-slate-800/70 to-transparent pointer-events-none transition-opacity duration-300 ${showRightFade ? 'opacity-100' : 'opacity-0'}`} aria-hidden="true" />
      </div>

      <div className="relative" ref={menuRef}>
        <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="flex items-center gap-2 p-1 rounded-full hover:bg-slate-200/50 dark:hover:bg-slate-700/50 transition-colors">
        ...
        <AnimatePresence>
            {isMenuOpen && (
                <motion.div
                    ...
                    className="absolute top-full right-0 mt-2 w-64 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 p-2 z-50"
                >
