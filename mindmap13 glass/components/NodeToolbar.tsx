...
const ToolbarButton = ({ icon, active = false, loading = false, onClick, title, disabled = false, 'data-tutorial-id': dataTutorialId, customClasses }: { icon: string; active?: boolean; loading?: boolean; onClick?: React.MouseEventHandler<HTMLButtonElement>; title: string; disabled?: boolean; 'data-tutorial-id'?: string; customClasses?: string }) => {
    ...
    const finalClasses = customClasses ? `${baseClasses} ${customClasses}` : `${baseClasses} ${activeClasses}`;
...
const NodeToolbar: React.FC<NodeToolbarProps> = (props) => {
  ...
  return (
    <div className="flex justify-center items-center">
        <div className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-md rounded-full p-1 flex items-center gap-1 shadow-md border border-white/20 dark:border-slate-700/50">
            ...
            <ToolbarButton icon="fa-vial-circle-check" onClick={onTestBranch} title="Test this Branch" disabled={isRoot} customClasses="bg-violet-500 text-white hover:bg-violet-600" />
            ...
            <div className="relative" onMouseEnter={() => setOpenMenu('select')} onMouseLeave={() => setOpenMenu(null)}>
                ...
                {openMenu === 'select' && (
                    <motion.div
                        ...
                        className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-lg shadow-xl border border-white/20 dark:border-slate-700/50 p-2 z-10"
                    >
            ...
            <div className="relative" onMouseEnter={() => setOpenMenu('style')} onMouseLeave={() => setOpenMenu(null)}>
                 ...
                 {openMenu === 'style' && (
                     <motion.div
                        ...
                        exit="hidden"
                        variants={dropdownVariants}
                        className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-lg shadow-xl border border-white/20 dark:border-slate-700/50 p-2 z-10"
                    >
