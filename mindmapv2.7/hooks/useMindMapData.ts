...
const createNewSubjectObject = (name: string, userId: string, color: string): Omit<MindMapDocument, 'id'> => {
  return {
    ...
    learningProfile: {
        ...
        interactionCount: 0,
        customPrompts: ["Explain this topic", "Summarize key points", "Give an example"],
    }
  };
};
...
const useMindMapData = (userId: string | null) => {
  ...
  const updateCustomPrompts = useCallback(async (prompts: string[]) => {
    if (!activeSubjectId) return;
    await db.collection('documents').doc(activeSubjectId).update({
        'learningProfile.customPrompts': prompts.slice(0, 3) // Enforce max 3
    });
  }, [activeSubjectId]);
