...
import { ExamConfig, Question, ExamResult, StudySprint, LearningProfile, GradedAnswer, VoiceCommandContext } from '../types';
...
async function generateSingleQuestionBatch(config: ExamConfig, prioritizedTopics: { focus: string[], review: string[] }, mindMapContext: string, documentsContent: string): Promise<Omit<Question, 'id'>[]> {
    const systemInstruction = `You are an expert exam creator and AI Tutor. Your task is to generate a personalized ${config.type} based EXCLUSIVELY on the student's provided learning materials. You MUST NOT use any external knowledge.`;
    
    const prompt = `Please generate an exam based STRICTLY on the following materials. Do not invent information or use any knowledge outside of what is provided here.

**Source Material 1: Mind Map Structure**
This is the student's mind map. All questions should be answerable using the topics and relationships shown here.
\`\`\`
${mindMapContext}
\`\`\`

**Source Material 2: Uploaded Documents**
This is the raw text from the student's documents. Use this as the primary source of truth for definitions, examples, and details.
\`\`\`
${documentsContent || 'No documents provided.'}
\`\`\`

**Exam Configuration & Topic Prioritization**
- Type: ${config.type}
- Number of Questions: ${config.numQuestions}
- Question Types: ${config.questionTypes.join(', ')}

The student needs more practice on the "Focus Areas". Generate MOST questions (~75%) from these topics.
**Focus Areas (Weak Topics):**
- ${prioritizedTopics.focus.join('\n- ') || 'None provided.'}

Generate FEWER questions (~25%) from the "Review Topics" to reinforce learning.
**Review Topics (Stronger Topics):**
- ${prioritizedTopics.review.join('\n- ') || 'None provided.'}

**CRITICAL Instructions:**
1.  **Strictly Adhere to Source:** Every question MUST be directly answerable from the provided "Mind Map Structure" and "Uploaded Documents". Do NOT use any outside information.
2.  Create exactly ${config.numQuestions} questions.
3.  The questions should be a mix of the requested types: ${config.questionTypes.join(', ')}.
4.  Heavily prioritize the "Focus Areas" when creating questions.
5.  For "multiple-choice", provide exactly 4 distinct options. The correct answer and distractors must be plausible based on the source material.
6.  For "true-false", the "options" field MUST be an array of two strings: ["True", "False"].
7.  For "fill-in-the-blank", the question should contain a blank placeholder (e.g., "____").
8.  **Crucial Mapping:** For every question, you MUST identify the most relevant concept from the "Mind Map Structure". Provide this concept's exact text in the "relatedNodeTopicText" field.
9.  **Hints:** Generate a helpful, one-sentence hint for each question that guides the student towards the correct answer using only information from the source material.
10. **JSON Output:** Your entire response MUST be a single, valid JSON object that adheres to the schema. Do not output any text or markdown formatting.`;
...
export async function generateExamQuestions(
    ...
    // A single API call is made to generate all questions at once, significantly reducing cost and latency.
    progressCallback(0.2, `Warming up the AI...`);
    progressCallback(0.5, `Generating ${totalQuestions} unique questions...`);
    
    // The config object already contains the total number of questions requested.
    const questions = await generateSingleQuestionBatch(config, prioritizedTopics, mindMapContext, documentsContent);
    
    progressCallback(1, "Finalizing questions...");
    
    // Safeguard: The AI might not return the exact number requested.
    // We deduplicate by question text and then slice to the desired amount to ensure correctness.
    const uniqueQuestions = Array.from(new Map(questions.map(q => [q.questionText, q])).values());
    
    return uniqueQuestions.slice(0, totalQuestions);
}

const gradedAnswerBatchSchema = {
    type: Type.OBJECT,
    properties: {
        results: {
            type: Type.ARRAY,
            description: "An array of grading results, one for each question in the batch. MUST be in the same order as the input.",
            items: {
                type: Type.OBJECT,
                properties: {
                    questionText: { type: Type.STRING, description: "The exact text of the question being graded, used for mapping." },
                    isCorrect: { type: Type.BOOLEAN },
                    explanation: { type: Type.STRING, description: "A brief, helpful explanation." }
                },
                required: ["questionText", "isCorrect", "explanation"]
            }
        }
    },
    required: ["results"]
...
async function gradeAnswerBatch(batch: QuestionWithAnswer[]): Promise<{ questionText: string; isCorrect: boolean; explanation: string; }[]> {
    const systemInstruction = `You are a helpful and encouraging AI Tutor. Your task is to grade a batch of student answers and provide clear, constructive explanations for each.`;

    const questionsAndAnswersString = batch.map((item, index) => `
---
Question ${index + 1}:
...
---`).join('\n');
    ...
    const prompt = `Please grade the following batch of answers for an exam.

${questionsAndAnswersString}

Instructions:
1. For each question, compare the "Student's Answer" to the "Correct Answer".
2. **Grading Rule:** Determine if the answer is correct. For short-answer questions, be forgiving of minor spelling mistakes or slightly different wording, as long as the core meaning is clearly correct. The student's intent is more important than perfect spelling.
3. Provide a helpful "explanation" for each. If incorrect, explain the right answer. If correct, offer a brief confirmation.
4. Your entire response MUST be a single, valid JSON object. It must contain a "results" array with an object for each question in the batch.
5. CRUCIAL: Each object in the "results" array must include the original "questionText" for mapping, and the results must be in the same order as the questions provided in this prompt.`;
    ...
    try {
        const response = await ai.models.generateContent({
            ...
            config: {
                systemInstruction,
                responseMimeType: "application/json",
                responseSchema: gradedAnswerBatchSchema,
                temperature: 0.3,
            }
        });
        const jsonText = response.text.trim();
        const result = safeJsonParse<{ results: { questionText: string; isCorrect: boolean; explanation: string; }[] }>(jsonText);
        if (result && Array.isArray(result.results)) {
            if (result.results.length !== batch.length) {
                console.warn(`AI returned ${result.results.length} results for a batch of ${batch.length}.`);
            }
            return result.results;
        }
        throw new Error("AI did not return a valid 'results' array.");
    } catch (error) {
        console.error("Error grading answer batch with Gemini:", error);
        throw new Error(`Failed to grade answer batch: ${(error as Error).message}`);
    }
}
...
export async function gradeAndAnalyzeExam(
    ...
    // Consolidate all questions and answers into a single list for one API call.
    const allQuestionsWithAnswers: QuestionWithAnswer[] = questions.map(q => ({
        question: q,
        userAnswer: userAnswers.get(q.id) || "Not answered"
    }));

    if (allQuestionsWithAnswers.length === 0) {
        return; // Nothing to grade.
    }

    try {
        // Make a single, efficient API call to grade all answers at once.
        const results = await gradeAnswerBatch(allQuestionsWithAnswers);

        // Process the results client-side, calling the progress callback for each.
        results.forEach(result => {
            const originalItem = allQuestionsWithAnswers.find(item => item.question.questionText === result.questionText);
            if (originalItem) {
                const gradedAnswer: GradedAnswer = {
                    questionText: originalItem.question.questionText,
                    userAnswer: originalItem.userAnswer,
                    correctAnswer: originalItem.question.correctAnswer,
                    isCorrect: result.isCorrect,
                    explanation: result.explanation,
                };
                onProgress(gradedAnswer);
            } else {
                 console.warn("Could not map a graded result back to its original question:", result);
            }
        });
    } catch (error) {
        console.error("The entire grading process failed:", error);
        // If the single batch fails, provide error feedback for all questions.
        allQuestionsWithAnswers.forEach(item => {
            const errorAnswer: GradedAnswer = {
                questionText: item.question.questionText,
                userAnswer: item.userAnswer,
                correctAnswer: item.question.correctAnswer,
                isCorrect: false,
                explanation: "An error occurred while grading this question. Please try submitting again."
            };
