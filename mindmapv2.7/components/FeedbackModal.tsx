...
const CategoryButton: React.FC<{ icon: string, text: string, onClick: () => void }> = ({ icon, text, onClick }) => (
    <button
        onClick={onClick}
        className="flex-1 p-6 bg-slate-100/50 dark:bg-slate-700/50 rounded-lg text-center hover:bg-slate-200/50 dark:hover:bg-slate-600/50 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500"
    >
    ...
    const renderStep = () => {
        switch (step) {
                    ...
                    <form onSubmit={handleSubmit}>
                         ...
                         <div className="space-y-4">
                            <input
                                ...
                                required
                                className="w-full p-2 border border-slate-300 dark:border-slate-600 bg-white/50 dark:bg-slate-800/50 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                            />
                             <textarea
                                value={description}
                            ...
                            <div className="flex items-center gap-4">
                                <button
                                    ...
                                    className="px-3 py-1.5 text-sm font-semibold text-slate-700 dark:text-slate-200 bg-slate-100/50 dark:bg-slate-700/50 hover:bg-slate-200/50 dark:hover:bg-slate-600/50 rounded-md disabled:opacity-50"
                                >
                         ...
                         <div className="mt-6 flex justify-end gap-3">
                            <button type="button" onClick={() => setStep('category')} className="px-4 py-2 text-sm font-semibold rounded-md hover:bg-slate-100/50 dark:hover:bg-slate-700/50">Back</button>
            ...
            case 'thanks':
                 return (
                    <div className="text-center py-12">
                        <div className="w-16 h-16 rounded-full bg-green-100/80 dark:bg-green-900/50 flex items-center justify-center mx-auto mb-4 text-3xl text-green-500">
            ...
            {isOpen && (
                ...
                >
                    <motion.div
                        ...
                        className="relative w-full max-w-lg glass-effect rounded-xl p-6"
                    >
                         <button onClick={handleClose} className="absolute top-3 right-3 w-8 h-8 rounded-full flex items-center justify-center text-slate-500 hover:bg-slate-100/50 dark:hover:bg-slate-700/50">
