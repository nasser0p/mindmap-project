...
const MobileToolbarButton: React.FC<{ icon: string; onClick: (e: React.MouseEvent) => void; disabled?: boolean; title: string; isActive?: boolean }> = ({ icon, onClick, disabled, title, isActive }) => (
  <button
    ...
    className={`w-full h-full flex items-center justify-center rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex-col text-xs gap-1.5 p-1 ${
        isActive ? 'text-blue-600 dark:text-blue-400 bg-blue-100/80 dark:bg-blue-900/50' : 'text-slate-600 dark:text-slate-200'
    }`}
  >
...
const MobileToolbar: React.FC<MobileToolbarProps> = (props) => {
  const { 
    toolMode, onToolChange, onUndo, onRedo, canUndo, canRedo, 
    onStartExam, masteryScore, onStartStudySprint, theme, onToggleTheme,
    onOpenAiAssistant, onZoomIn, onZoomOut, onZoomToFit, onSendFeedback
  } = props;
      ...
      <AnimatePresence>
        {isMoreMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 20 }}
            className="absolute bottom-full left-1/2 -translate-x-1/2 w-[calc(100%-16px)] mb-2 p-2 glass-effect rounded-xl"
          >
            <div className="grid grid-cols-4 gap-2">
                ...
                <MobileToolbarButton icon="fa-search-plus" onClick={onZoomIn} title="Zoom In" />
                <MobileToolbarButton icon="fa-search-minus" onClick={onZoomOut} title="Zoom Out" />
                <MobileToolbarButton icon="fa-compress" onClick={onZoomToFit} title="Fit" />
                ...
                <MobileToolbarButton icon="fa-comment-dots" onClick={onSendFeedback} title="Feedback" />
            </div>
          </motion.div>
        )}
      </AnimatePresence>

      <div className="glass-effect p-1.5 border-t shadow-[0_-5px_15px_-5px_rgba(0,0,0,0.1)]">
        <div className="grid grid-cols-5 h-16 gap-1.5">
            ...
            <MobileToolbarButton icon="fa-wand-magic-sparkles" onClick={onOpenAiAssistant} title="Ask AI" />
