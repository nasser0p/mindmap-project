...
const ToolbarButton: React.FC<{ icon: string; onClick: () => void; disabled?: boolean; title: string; isActive?: boolean; 'data-tutorial-id'?: string; className?: string; }> = ({ icon, onClick, disabled, title, isActive = false, 'data-tutorial-id': dataTutorialId, className = '' }) => (
  <button
    onClick={onClick}
    disabled={disabled}
    title={title}
...
const Toolbar: React.FC<ToolbarProps> = (props) => {
    ...
    return (
        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-2">
            <motion.div
                layout
                className="relative glass-effect rounded-xl p-1 flex items-center gap-1"
                initial={{ width: 'auto' }}
                ...
                <AnimatePresence>
                    {isExpanded && (
                        ...
                        >
                                            ...
                                            <AnimatePresence>
                                                {isColorPickerVisible && (
                                                    <motion.div
                                                        ...
                                                        className="absolute top-full left-0 mt-2 w-48 glass-effect rounded-lg p-2 z-10"
                                                    >
                            ...
                            <button onClick={rest.onZoomToFit} className="px-3 h-10 text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-200/50 dark:hover:bg-slate-700/50 rounded-lg" title="Fit to Screen">
