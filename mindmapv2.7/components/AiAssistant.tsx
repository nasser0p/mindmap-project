...
interface AiAssistantProps {
    ...
    onUpdateCustomPrompts: (prompts: string[]) => void;
    onGenerateNodesFromText: (text: string, messageId: string) => void;
    generatingNodesFromChat: string | null;
}
...
const ChatBubble: React.FC<{
    message: ChatMessage;
    onGenerateNodes: (text: string, messageId: string) => void;
    generatingNodesId: string | null;
    isAiReplying: boolean;
}> = ({ message, onGenerateNodes, generatingNodesId, isAiReplying }) => {
    ...
    const showGenerateButton = !isUser && message.text.length > 50;
    const isGenerating = generatingNodesId === message.id;
    ...
    return (
        ...
        >
            <div className={`flex items-start gap-2 w-full ${isUser ? 'justify-end' : 'justify-start'}`}>
                {!isUser && (
                ...
                <div className={`group relative max-w-[85%] p-3 rounded-2xl text-sm ${
                    ...
                    : 'bg-white/50 dark:bg-slate-700/60 backdrop-blur-sm border border-white/30 dark:border-slate-600/50 rounded-bl-lg'
                }`}>
            ...
            {showGenerateButton && (
                ...
                >
                    <button
                        onClick={() => onGenerateNodes(message.text, message.id)}
                        disabled={isAiReplying || (!!generatingNodesId && !isGenerating)}
                        className="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold bg-white/60 dark:bg-slate-700/60 backdrop-blur-sm border border-slate-300/70 dark:border-slate-600/70 rounded-full hover:bg-slate-50/80 dark:hover:bg-slate-600/80 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {isGenerating ? (
                        ...
                        <span>{isGenerating ? 'Generating...' : 'Add as Nodes'}</span>
                    </button>
                </motion.div>
            )}
        </motion.div>
...
const AiAssistant: React.FC<AiAssistantProps> = (props) => {
    const { 
        ...
        customPrompts, onUpdateCustomPrompts,
        onGenerateNodesFromText, generatingNodesFromChat
    } = props;
    ...
    const handleSavePrompts = () => {
        onUpdateCustomPrompts(editedPrompts.map(p => p.trim()).filter(Boolean));
                            ...
                            <>
                                ...
                                <div ref={chatContainerRef} className="flex-1 overflow-y-auto -mx-2 px-2 space-y-4">
                                    {chatHistory.map((msg, index) => (
                                        <ChatBubble
                                            key={msg.id || index}
                                            message={msg}
                                            onGenerateNodes={onGenerateNodesFromText}
                                            generatingNodesId={generatingNodesFromChat}
                                ...
                                <form onSubmit={handleChatSubmit} className="mt-4 flex-shrink-0">
                                        ...
                                        <AnimatePresence mode="wait">
                                            {isEditingPrompts ? (
                                                <motion.div
                                                    ...
                                                    exit={{ opacity: 0, y: 10 }}
                                                    className="p-3 bg-white/20 dark:bg-black/10 rounded-lg border border-slate-300/50 dark:border-slate-600/50"
                                                >
                                                    <p className="text-xs font-semibold mb-2 text-slate-600 dark:text-slate-300">Edit quick prompts</p>
                                                    <div className="space-y-2">
                                                        {editedPrompts.map((prompt, index) => (
                                                            <input
                                                                ...
                                                                className="w-full px-2 py-1 text-sm bg-white/50 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-600 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-400"
                                                            />
                                                        ))}
                                                    </div>
                                                    <div className="flex items-center justify-end gap-2 mt-3">
                                                        <button type="button" onClick={handleCancelEditPrompts} className="px-3 py-1 text-xs font-semibold rounded-md hover:bg-slate-100/50 dark:hover:bg-slate-700/50">Cancel</button>
                                                        <button type="button" onClick={handleSavePrompts} className="px-3 py-1 text-xs font-semibold bg-blue-500 text-white rounded-md hover:bg-blue-600">Save</button>
                                                    </div>
                                                </motion.div>
                                            ) : (
                                                <motion.div
                                                ...
                                                >
                                                    {customPrompts.map((prompt, index) => (
                                                        <button
                                                            ...
                                                            className="px-3 py-1.5 text-xs font-semibold bg-white/60 dark:bg-slate-700/60 backdrop-blur-sm border border-slate-300/70 dark:border-slate-600/70 rounded-full hover:bg-slate-50/80 dark:hover:bg-slate-600/80 transition-colors disabled:opacity-50"
                                                        >
                                                            {prompt}
                                                        </button>
                                                    ))}
                                                    <button
                                                        ...
                                                        className="w-7 h-7 flex items-center justify-center rounded-full text-slate-500 dark:text-slate-400 bg-slate-200/50 dark:bg-slate-700/50 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
                                                        title="Edit prompts"
                                                    >
                                    ...
                                    <div className="relative">
                                        <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder={isSingleNodeSelected ? `Ask about "${selectedNodes[0].text}"...` : `Ask about these ${selectedNodes.length} topics...`} className="w-full pl-4 pr-12 py-2.5 bg-white/50 dark:bg-slate-700/50 backdrop-blur-sm border border-slate-300/70 dark:border-slate-600/70 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-400 transition-shadow text-sm" disabled={isAiReplying} />
