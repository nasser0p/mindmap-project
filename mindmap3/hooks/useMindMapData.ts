...
const useMindMapData = (userId: string | null) => {
  ...
  const addChildNode = useCallback((parentId: string, text: string) => {
    ...
    const newNode: MindMapNode = {
      ...
      text,
      children: [],
      color: parentNode.id === activeDocument.root.id ? NODE_COLORS[(parentNode.children?.length || 0) % NODE_COLORS.length] : parentNode.color,
      attachments: [],
      masteryScore: 0,
    };
  ...
  const addMultipleChildrenNode = useCallback((parentId: string, ideas: { text: string }[]) => {
    ...
    const newNodes: MindMapNode[] = ideas.map((idea, index) => ({
      ...
      text: idea.text,
      children: [],
      color: parentNode.id === activeDocument.root.id ? NODE_COLORS[(baseChildCount + index) % NODE_COLORS.length] : parentNode.color,
      attachments: [],
      masteryScore: 0,
    }));
  ...
  const addNodeWithChildren = useCallback((parentId: string, newNodeData: MindMapNodeData) => {
    ...
    let parentDepth = -1;
    hierarchy(activeDocument.root).each(d => {
        if (d.data.id === parentId) {
            parentDepth = d.depth;
        }
    });
    if (parentDepth === -1) return; // Parent not found, should not happen

    // Recursive helper to build the node tree with correct IDs and new color logic
    const assignIdsAndColors = (nodeData: MindMapNodeData, baseColor: string, childIndex: number, level: number = 0): MindMapNode => {
        let currentBaseColor = baseColor;
        // If the direct parent is the root, this new node defines a new branch color
        if (parentDepth === 0 && level === 0) {
            currentBaseColor = NODE_COLORS[childIndex % NODE_COLORS.length];
        }

        const nodeDepth = parentDepth + 1 + level;
        const finalColor = determineNodeColorByType(currentBaseColor, nodeData.type, nodeDepth);
        ...
        if (nodeData.children) {
            // Pass the UNMODIFIED base color to all descendants
            newNode.children = nodeData.children.map((child, index) => 
                assignIdsAndColors(child, currentBaseColor, index, level + 1)
            );
        }
        return newNode;
    };
    ...
    // Find the true base color for this branch by traversing up to the root's child
    let branchBaseColor = parentNode.color;
    if (parentDepth > 0) {
        let current = parentNode;
        let parent = findParentNode(current.id);
        while (parent && parent.id !== activeDocument.root.id) {
            current = parent;
            parent = findParentNode(current.id);
        }
        // `current` is now the main branch node (child of root), its color is the base color
        branchBaseColor = current.color;
    }
    
    const fullyFormedNode = assignIdsAndColors(newNodeData, branchBaseColor, baseChildCount);
    ...
    updateMindMap({ root: newRoot });
  }, [activeDocument, findNode, updateMindMap, findParentNode]);
