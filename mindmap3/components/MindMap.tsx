...
import { MindMapNode as MindMapNodeData, MindMapLink, GlowNode } from '../types';
...
interface MindMapProps {
  ...
  onActivateHotspot: (nodeId: string) => void;
...
const MindMap = forwardRef<MindMapActions, MindMapProps>((props, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    zoomIn: () => {
      if (zoomBehaviorRef.current && svgRef.current) {
        select(svgRef.current).transition().duration(250).call(zoomBehaviorRef.current.scaleBy, 1.2);
      }
    },
    zoomOut: () => {
      if (zoomBehaviorRef.current && svgRef.current) {
        select(svgRef.current).transition().duration(250).call(zoomBehaviorRef.current.scaleBy, 0.8);
      }
    },
    zoomToFit: () => {
  ...
  const activeHotspotNodeData = useMemo(() => originalNodesMap.get(activeHotspotNodeId || '') || null, [originalNodesMap, activeHotspotNodeId]);
  ...
  }, [isInitialTransformSet, throttledSetTransform, onTransformChange, toolMode]);
    ...
    <>
      ...
      >
        ...
        <g ref={gRef} className="zoom-container">
          ...
          <AnimatePresence>
            {selectedNodeIds.size === 1 && lastSelectedNodeId && selectedNodeData && nodePositions.get(lastSelectedNodeId) && !activeHotspotNodeId && (
              <foreignObject
          ...
          <AnimatePresence>
            {activeHotspotNodeId && activeHotspotNodeData && nodePositions.get(activeHotspotNodeId) && (
                 <foreignObject
                    x={(nodePositions.get(activeHotspotNodeId)!.x) - (HOTSPOT_WIDTH / 2)}
                    y={(nodePositions.get(activeHotspotNodeId)!.y) - (nodeSizes[activeHotspotNodeId]?.height || minNodeHeight) / 2 - HOTSPOT_HEIGHT - TOOLBAR_Y_OFFSET}
                    width={HOTSPOT_WIDTH}
