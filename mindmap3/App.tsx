...
const TopicHotspot = lazy(() => import('./components/TopicHotspot'));
...
const App: React.FC = () => {
  ...
  const isReviewModeActive = useMemo(() => isInGuidedReview || !!activeHotspotNodeId, [isInGuidedReview, activeHotspotNodeId]);
  ...
  const handleStartStudySprint = useCallback(async (duration: number) => {
    ...
    // --- Topic Hotspot Logic ---
    const handleActivateHotspot = useCallback((nodeId: string) => {
        setSelectedNodeIds(new Set());
        setActiveHotspotNodeId(nodeId);
    ...
    const handleEndGuidedReview = useCallback(() => {
        ...
        // Close hotspot directly without triggering the check again
        setActiveHotspotNodeId(null);
    ...
    const handleCloseHotspot = useCallback(() => {
        setActiveHotspotNodeId(null);
        setHotspotContent(null);
        // If user manually closes the hotspot during a guided tour, end the tour.
        if (isInGuidedReview) {
    ...
    const handleAdvanceGuidedReview = useCallback(() => {
        // 1. Mark current node as reviewed by removing its glow
        const reviewedNodeId = guidedReviewPath[guidedReviewIndex].nodeId;
        setGlowingNodes(prev => prev.filter(n => n.nodeId !== reviewedNodeId));

        // 2. Move to next node
        const nextIndex = guidedReviewIndex + 1;

        if (nextIndex >= guidedReviewPath.length) {
            // Last node was just reviewed, end the tour.
            handleEndGuidedReview();
        } else {
            setGuidedReviewIndex(nextIndex);
            const nextNode = guidedReviewPath[nextIndex];
            setNodeToCenterOn(nextNode.nodeId);
            // handleActivateHotspot closes the old one and opens the new one.
            handleActivateHotspot(nextNode.nodeId);
        }
    }, [guidedReviewPath, guidedReviewIndex, handleEndGuidedReview, handleActivateHotspot]);
    ...
    const handleStartGuidedReview = useCallback(() => {
        ...
        // Sort nodes by severity ('high' first) to prioritize review
        ...
        // Center on and activate the first node in the path
        const firstNode = sortedNodes[0];
        setNodeToCenterOn(firstNode.nodeId);
        handleActivateHotspot(firstNode.nodeId);
    }, [glowingNodes, handleActivateHotspot]);

    const hotspotData: { node: MindMapNode; incorrectQuestions: GradedAnswer[]; content: HotspotContent | null } | null = useMemo(() => {
            ...
            <AnimatePresence>
              {hotspotData && (
                <TopicHotspot
                  ...
                  onMarkAsReviewed={() => handleMarkAsReviewed(hotspotData.node.id)}
                  onExplainDifferently={() => handleHotspotExplain(hotspotData.node.text)}
                  onQuizAgain={() => handleHotspotQuiz(hotspotData.node.text)}
                  onAdvance={handleAdvanceGuidedReview}
                  onBackToMain={() => setHotspotContent(prev => prev ? { ...prev, view: 'main' } : null)}
                />
              )}
            </AnimatePresence>
          </Suspense>
