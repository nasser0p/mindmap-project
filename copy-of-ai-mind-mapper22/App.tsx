...
const MultiNodeToolbar: React.FC<MultiNodeToolbarProps> = ({ count, onDelete, onSetColor }) => {
    ...
    return (
        <motion.div
            className="absolute z-20 bottom-20 left-1/2 -translate-x-1/2 md:bottom-6 md:left-1/2 md:-translate-x-1/2 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md rounded-lg shadow-lg p-1 flex items-center gap-1 border border-slate-200/80 dark:border-slate-700/80"
                ...
                <AnimatePresence>
                    {isColorPickerOpen && (
                        <motion.div
                            ...
                            className="absolute bottom-full left-0 mb-2 w-48 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 p-2 z-10"
                        >
...
const App: React.FC = () => {
  ...
  }, [examState.view, studySprintState.view, dataActions, lastSelectedNodeId, selectedNodeIds, toolMode]);
  ...
  }, [activeDocument, findParentNodeFromHook, insertNodeBetween]);
  ...
  }, [currentUser, activeDocument, addSourceDocument, updateSourceDocument]);
  ...
  const handleGenerateNodesFromFile = useCallback(async (file: SourceDocumentFile) => {
    ...
    try {
        const response = await fetch('/api/process-document', {
            ...
            body: JSON.stringify({ downloadURL: file.downloadURL, mimeType: file.mimeType })
        });
        if (!response.ok) throw new Error(`Server responded with ${response.status}`);
        const { text, base64Data } = await response.json();
  ...
  const handleStartExam = useCallback(async (config: ExamConfig) => {
      ...
      try {
          ...
          const mindMapContext = allNodes.map(n => `- ${n.text}`).join('\n');

          // For simplicity, we'll just use document names as context.
          // A more advanced implementation would fetch and use document content.
          ...
          const questionsWithIds: Question[] = generatedQuestions.map(q => {
              const id = uuidv4();
              const relatedNode = allNodes.find(n => n.text === q.relatedNodeTopicText);
              if (relatedNode) {
  ...
  const handleSubmitExam = useCallback(async (answers: Map<string, string>, revealedHints: Set<string>) => {
      ...
      try {
          ...
          // Set glowing nodes for incorrect topics
          const incorrectNodeIds = results.analysis
              .filter(res => !res.isCorrect)
              .map(res => {
                  const q = examState.questions.find(q => q.questionText === res.questionText);
                  return q ? examState.questionToNodeIdMap.get(q.id) : null;
              })
              .filter((id): id is string => id !== null);

          setGlowingNodeIds(Array.from(new Set(incorrectNodeIds)));
  ...
  }, [examState.questions, examState.questionToNodeIdMap, dataActions, activeDocument]);
    ...
    <div className={`w-screen h-screen overflow-hidden flex flex-col font-sans ${theme}`}>
      ...
      <Suspense fallback={<ModalLoadingFallback />}>
          ...
          <ExamModal
              ...
              onClose={() => setExamState({ view: 'closed', config: null, questions: [], results: null, questionToNodeIdMap: new Map() })}
          />
          <StudySprintModal
