...
const MindMap = forwardRef<MindMapActions, MindMapProps>((props, ref) => {
  ...
  useEffect(() => {
    ...
    const selectionDrag = drag<SVGSVGElement, unknown>()
        ...
        .on('start', (event) => {
            ...
            const [x, y] = pointer(event.sourceEvent, gRef.current!);
            startPos = { x, y };
        ...
        .on('drag', (event) => {
            if (!startPos) return;
            const [cursorX, cursorY] = pointer(event.sourceEvent, gRef.current!);
  ...
  useEffect(() => {
    ...
    svg.transition().duration(400).ease(easeCubicOut).call(zoomBehaviorRef.current.transform, newTransform);
