...
const SubjectTab: React.FC<{
  ...
  const baseClasses = 'relative group flex items-center h-10 px-4 rounded-t-lg cursor-pointer transition-all duration-200 border-t-4';
...
const SubjectTabs: React.FC<SubjectTabsProps> = ({ documents, activeDocumentId, user, onSwitch, onAdd, onDelete, onRename }) => {
  ...
  return (
    <header className="bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 px-4 pt-2 flex items-center justify-between">
      <div className="flex items-center gap-1">
        ...
        <button 
          onClick={onAdd} 
          className="w-8 h-8 flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full transition-colors"
