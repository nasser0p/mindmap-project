...
const Node: React.FC<NodeProps> = (props) => {
  ...
  useLayoutEffect(() => {
    ...
    const requiredHeight = Math.max(minNodeHeight, nodeEl.scrollHeight);
    
    // Only auto-adjust if content is overflowing OR if the node has never been sized before.
    if (requiredHeight > size.height || nodeData.height === undefined) {
        // We have a change that needs to be applied and possibly persisted.
        const newSize = { width: size.width, height: requiredHeight };
        setSize(newSize);
        onSizeChange(id, newSize);
        
        // Persist if the stored value is different.
        if (Math.abs((nodeData.height || 0) - requiredHeight) > 1) {
            onUpdateNodeSize(id, newSize.width, newSize.height);
        }
    } else {
        // No overflow, the current size is fine (it might be user-defined).
