...
const SubjectTab: React.FC<{
  ...
  // Use a mock color for the top border since color is now chapter-specific
  const mockColor = '#a855f7'; // A default color from the palette
  ...
  const activeClasses = 'bg-slate-100 dark:bg-slate-900 shadow-sm text-slate-800 dark:text-slate-100';
  const inactiveClasses = 'text-slate-500 dark:text-slate-400 hover:bg-slate-200/50 dark:hover:bg-slate-800/50';

  const baseClasses = 'relative group flex-shrink-0 flex items-center h-10 px-4 rounded-t-lg cursor-pointer transition-all duration-200';
  ...
  return (
    ...
    >
      {isEditing ? (
        ...
        <span className="font-semibold select-none truncate w-32 text-left">
          {doc.name}
        </span>
      )}
      {!isEditing && (
