...
const createNewSubjectObject = (name: string, userId: string): Omit<MindMapDocument, 'id'> => {
  return {
    name,
    ownerId: userId,
    sourceDocuments: [],
...
const useMindMapData = (userId: string | null) => {
  ...
  const addSubject = useCallback(async () => {
    ...
    const newSubjectData = createNewSubjectObject('New Subject', userId);
    try {
        const docRef = await db.collection('documents').add(newSubjectData);
        // Also create a default first chapter
        const firstChapterData = createNewChapterObject('Main Chapter', NODE_COLORS[subjects.length % NODE_COLORS.length], 0);
