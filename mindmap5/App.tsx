...
const App: React.FC = () => {
  ...
  const handleGenerateIdeas = useCallback(async (nodeId: string) => {
    const node = dataActions.findNode(nodeId);
    if (!node || !activeSubject) return;
    trackUserAction('GENERATE_IDEAS');
    setGeneratingIdeasForNodeId(nodeId);
    try {
        const ideas = await generateIdeasForNode(node.text, activeSubject.learningProfile);
  ...
  }, [dataActions, activeSubject, trackUserAction]);
  ...
  return (
    <div className={`w-screen h-screen overflow-hidden flex flex-col font-sans ${theme}`}>
      ...
      />
      <main className="flex-1 relative bg-slate-100 dark:bg-slate-900 flex">
        {activeSubject && (
            <ChapterSidebar
                chapters={chapters}
        ...
        <div className="flex-1 relative">
            ...
            {activeChapter ? (
              ...
              <MindMap
                ...
                onSelectBranch={() => handleSelectBranch(lastSelectedNodeId!)}
                onSelectChildren={() => handleSelectChildren(lastSelectedNodeId!)}
                onSelectSiblings={() => handleSelectSiblings(lastSelectedNodeId!)}
                ...
                onSelectionEnd={() => { if(toolMode === 'select') handleToolChange('pan'); }}
