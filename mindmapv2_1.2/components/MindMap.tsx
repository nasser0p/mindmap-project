...
const MindMap = forwardRef<MindMapActions, MindMapProps>((props, ref) => {
  ...
  // State to manage which node is currently in inline-editing mode
  ...
  // When the document changes, exit any active editing state
  useEffect(() => {
  ...
  const handleStartEdit = useCallback((nodeId: string) => {
    ...
    // When starting to edit, deselect other nodes for a cleaner UI
  ...
  useImperativeHandle(ref, () => ({
    ...
    zoomToFit: () => {
        ...
        // FIX: The getBBox method on SVG elements requires an options object argument.
  ...
  useEffect(() => {
    if (d3Root) {
        ...
        setNodePositions(prevPositions => {
            ...
            const allNodeIds = new Set<string>(hierarchy<MindMapNodeData>(root).descendants().map(d => d.data.id));
          ...
          longPressTimerRef.current = setTimeout(() => {
            if (isAwaitingLongPressRef.current) { // Check if not cancelled by movement
              ...
              if (typeof navigator.vibrate === 'function') {
                // FIX: The 'vibrate' method requires a duration or pattern argument.
                navigator.vibrate(50);
              }
              const currentTransform = zoomTransform(svgRef.current!);
              // FIX: The d3-zoom invert method requires a point array [x, y] to invert.
    ...
    const dragBehavior = drag<HTMLDivElement, HierarchyPointNode<MindMapNodeData>>()
      ...
      .on('start', function (this: HTMLDivElement, event: DragEvent, d: HierarchyPointNode<MindMapNodeData>) {
        ...
        userSelection.forEach(id => {
            const nodeData = findNodeById(root, id);
      ...
      .on('drag', function (this: HTMLDivElement, event: DragEvent, d: HierarchyPointNode<MindMapNodeData>) {
        ...
        if (userDragSelectionRef.current.size === 1) { // Reparenting only for single node drags
            const [worldX, worldY] = transformRef.current.invert([event.x, event.y]);
            const hitTarget = nodes.find(targetNode => {
              ...
              // Hitbox check in world space
              return worldX > targetPos.x - nodeWidthForHitbox / 2 && worldX < targetPos.x + nodeWidthForHitbox / 2 && worldY > targetPos.y - nodeHeightForHitbox / 2 && worldY < targetPos.y + nodeHeightForHitbox / 2;
            });
      ...
      .on('end', function (this: HTMLDivElement, event: DragEvent, d: HierarchyPointNode<MindMapNodeData>) {
        ...
        // Use the ref of the user's intended selection to decide on action
        if (finalDropTargetId && userDragSelectionRef.current.size === 1) {
        ...
        dragStartOffsetsRef.current.clear();
        userDragSelectionRef.current.clear(); // Clean up
      });
  ...
  }, [nodes, d3Root, onNodeSelect, onNodeDragStart, onMultipleNodePositionsUpdate, onNodeMove, isDescendant, focusedNodeId, root, isReviewModeActive, getAllDescendantIds]);
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    // Handle F2 and Enter to start editing a selected node
  ...
  }, [selectedNodeIds, root.id, selectedLinkId, editingNodeId, onDeleteNodes, onDeleteLink]);

  const handleMouseMove = (e: React.MouseEvent) => {
    if (drawingLinkState && gRef.current) {
      // FIX: Preserve the tuple type from `pointer` to avoid type errors with `invert`.
      const screenPoint = pointer(e, svgRef.current!);
      const inverted = transformRef.current.invert(screenPoint);
  ...
  const handleNodeSelect = useCallback((id: string, event: React.MouseEvent) => {
      ...
      if (isShift) {
          ...
          // The logic to enter edit mode on a second click is now handled within the Node component itself.
  ...
  return (
    <div className="w-full h-full relative overflow-hidden dotted-background">
      ...
      >
          {nodes.map(d => {
            ...
            return (
              <Node
                  ...
                  isEditingMode={editingNodeId === d.data.id}
          ...
          <AnimatePresence>
            {selectedNodeIds.size === 1 && lastSelectedNodeId && selectedNodeData && !editingNodeId && nodePositions.get(lastSelectedNodeId) && (
              (() => {
