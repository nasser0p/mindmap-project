...
const MindMap = forwardRef<MindMapActions, MindMapProps>((props, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    zoomToFit: () => {
        ...
        // @FIX: The getBBox method on SVG elements requires an options object, even if empty, to avoid an error.
    ...
    const selectionDrag = drag<SVGSVGElement, unknown>()
      ...
      .on('start', (event) => {
        ...
        if (isTouchEvent) {
          ...
          // @FIX: The d3.pointer function can return undefined if coordinates can't be determined.
          // Check for a valid result before destructuring to prevent a runtime error.
          const touchPoint = pointer(event, svgRef.current!);
          ...
          setIsBoxSelecting(true);
          const currentTransform = zoomTransform(svgRef.current!);
          // @FIX: The d3.pointer function can return undefined. Check for a valid result before using it.
