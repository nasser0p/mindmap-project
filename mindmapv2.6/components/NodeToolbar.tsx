...
const ToolbarButton = ({ icon, active = false, loading = false, onClick, title, disabled = false, 'data-tutorial-id': dataTutorialId }: { icon: string; active?: boolean; loading?: boolean; onClick?: React.MouseEventHandler<HTMLButtonElement>; title: string; disabled?: boolean; 'data-tutorial-id'?: string; }) => {
    ...
    const activeClasses = active ? 'bg-blue-500 text-white' : 'bg-white/80 dark:bg-slate-700/80 text-slate-600 dark:text-slate-300 hover:bg-slate-100/80 dark:hover:bg-slate-600/80';
    return (
        <button
            onClick={onClick}
...
const DropdownMenuItem = ({ icon, text, onClick, loading = false, disabled = false }: { icon: string; text: string; onClick: () => void; loading?: boolean; disabled?: boolean; }) => (
    <button
        onClick={onClick}
        disabled={loading || disabled}
        className="w-full flex items-center gap-3 px-3 py-2 text-left text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100/50 dark:hover:bg-slate-800/50 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
    >
            ...
            <div className="relative" onMouseEnter={() => setOpenMenu('select')} onMouseLeave={() => setOpenMenu(null)}>
                ...
                {openMenu === 'select' && (
                    <motion.div
                        ...
                        className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 glass-effect rounded-lg p-2 z-10"
                    >
            ...
            <div className="relative" onMouseEnter={() => setOpenMenu('style')} onMouseLeave={() => setOpenMenu(null)}>
                 ...
                 {openMenu === 'style' && (
                     <motion.div
                        ...
                        exit="hidden"
                        variants={dropdownVariants}
                        className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 glass-effect rounded-lg p-2 z-10"
                    >
