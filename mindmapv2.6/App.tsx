...
const App: React.FC = () => {
  ...
  const displayRoot = useMemo(() => {
    ...
    // Note: We don't need a deep clone here because the NavigationPanel only reads children,
    // and the MindMap component's layout logic handles collapsed nodes separately.
  ...
  const branchMasteryScores = useMemo(() => {
    ...
    const nodeMap = new Map(allNodesInChapter.map(n => [n.id, n]));
    ...
    allNodesInChapter.forEach(node => {
        // This is a heavy operation if not memoized, but `useMemo` handles it.
  ...
  const handleStartExam = useCallback(async (config: ExamConfig) => {
      ...
      try {
          ...
          // Adaptive Logic: Categorize topics by mastery score
          const focusTopics = nodesForExam
              .filter(n => (n.masteryScore || 0) < 0.6)
              .map(n => n.text);
          
          const reviewTopics = nodesForExam
              .filter(n => (n.masteryScore || 0) >= 0.6)
              .map(n => n.text);

          // Handle edge cases where one list is empty to ensure AI gets good context
          if (focusTopics.length === 0 && reviewTopics.length > 0) {
              ...
              for (let i = 0; i < topicsToMove; i++) {
                  const randomIndex = Math.floor(Math.random() * reviewTopics.length);
                  focusTopics.push(reviewTopics.splice(randomIndex, 1)[0]);
              }
          } else if (reviewTopics.length === 0 && focusTopics.length > 0) {
              const topicsToMove = Math.max(1, Math.floor(focusTopics.length * 0.25));
              for (let i = 0; i < topicsToMove; i++) {
                  const randomIndex = Math.floor(Math.random() * focusTopics.length);
                  reviewTopics.push(focusTopics.splice(randomIndex, 1)[0]);
              }
          }
          ...
          const progressCallback = (progress: number, message: string) => {
              dispatchExam({ type: 'SET_PROGRESS', payload: { progress, message } });
          };
          
          const generatedQuestions = await generateExamQuestions(config, prioritizedTopics, documentsContext, progressCallback);
          ...
          dispatchExam({ type: 'GENERATION_SUCCESS', payload: { questions: questionsWithIds, questionToNodeIdMap: questionToNodeIdMap } });
      } catch (error) {
  ...
  const handleSubmitExam = useCallback(async (answers: Map<string, string>, revealedHints: Set<string>) => {
    ...
    try {
        const onProgressCallback = (gradedAnswer: GradedAnswer) => {
            dispatchExam({ type: 'ADD_GRADED_ANSWER', payload: { gradedAnswer } });
        };
  ...
  // This useEffect handles the side-effects of a successful exam grading.
  // It's decoupled from the submission action itself for better state management.
  useEffect(() => {
  ...
  const handleStartStudySprint = useCallback(async (duration: number) => {
    ...
    const placeholderDoc: SourceDocumentFile = {
        id: fileId,
        name: file.name,
  ...
  const handleGenerateEnhancedMindMap = useCallback(async (fileId: string) => {
    ...
    try {
        ...
        // FIX: Added missing arguments to generateEnhancedMindMapFromFile call.
        // The original call was missing the base64 data and mime type, causing an error.
// FIX: Pass all required arguments to generateEnhancedMindMapFromFile.
        ...
        if (newNodes && newNodes.length > 0) {
            newNodes.forEach(nodeData => {
                addNodeWithChildren(lastSelectedNodeId, nodeData);
            });
  ...
  const handleFinishReview = useCallback(() => {
    ...
    }, [activeHotspotNodeId, examState.results, examState.questions, examState.questionToNodeIdMap, dataActions, hotspotContent]);
    ...
    <div className={`w-screen h-screen overflow-hidden flex flex-col font-sans ${theme}`}>
      ...
      <main className="flex-1 relative bg-slate-100 dark:bg-slate-900 flex flex-col">
        ...
        <div className="flex-1 relative">
            {!isMobile && activeChapter && displayRoot && (
