...
type ExamState = {
    view: 'closed' | 'scope' | 'config' | 'loading' | 'active' | 'results';
...
type ExamAction =
  ...
  | { type: 'GRADING_SUCCESS'; payload: ExamResult }
  | { type: 'FAIL_AND_CLOSE' }
...
function examReducer(state: ExamState, action: ExamAction): ExamState {
    switch (action.type) {
        ...
        case 'START_GENERATION':
            return { ...state, config: action.payload, view: 'loading' };
        case 'GENERATION_SUCCESS':
            return { ...state, questions: action.payload.questions, questionToNodeIdMap: action.payload.questionToNodeIdMap, view: 'active' };
        case 'SUBMIT':
            return { ...state, view: 'loading', revealedHints: action.payload.revealedHints };
        case 'GRADING_SUCCESS':
            return { ...state, results: action.payload, view: 'results' };
        case 'FAIL_AND_CLOSE':
...
const App: React.FC = () => {
  ...
  const handleSubmitExam = useCallback(async (answers: Map<string, string>, revealedHints: Set<string>) => {
      ...
      try {
          ...
          dispatchExam({ type: 'GRADING_SUCCESS', payload: results });
      } catch (error) {
  ...
  return (
    <div className={`w-screen h-screen overflow-hidden flex flex-col font-sans ${theme}`}>
      ...
      <Suspense fallback={<ModalLoadingFallback />}>
          ...
          <ExamModal
              state={{
                // Fix: Prevent passing 'scope' view to ExamModal which doesn't handle it
                view: examState.view === 'scope' ? 'closed' : examState.view,
