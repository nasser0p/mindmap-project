...
const Node: React.FC<NodeProps> = (props) => {
  ...
  useLayoutEffect(() => {
    ...
    if (!nodeEl || isResizing) {
        onSizeChange(id, size);
        return;
    }

    const contentHeight = Math.max(minNodeHeight, nodeEl.scrollHeight);
    const currentPersistedHeight = nodeData.height || minNodeHeight;

    if (isEditing) {
        // While editing, grow to fit content in real-time.
        const newHeight = Math.max(size.height, contentHeight);
        if (Math.abs(newHeight - size.height) > 1) {
            const newSize = { width: size.width, height: newHeight };
            setSize(newSize);
        ...
        // When not editing, the source of truth is the persisted height from the database.
        // This allows manual shrinking to stick. If an external text update makes
        // content taller, it will overflow until the user edits or manually resizes the node.
        if (Math.abs(size.height - currentPersistedHeight) > 1) {
            const newSize = { width: size.width, height: currentPersistedHeight };
            setSize(newSize);
            onSizeChange(id, newSize);
        } else {
            onSizeChange(id, size);
  ...
  const handleBlur = () => {
    if (text.trim() === '' && !image) {
        ...
        // Persist both text and size, as either could have changed during editing.
        onUpdate(id, text);
        onUpdateNodeSize(id, size.width, size.height);
    }
    setIsEditing(false);
    onEditComplete();
  };
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        overflow: 'hidden', // Changed from visible to hidden to contain overflowing text
