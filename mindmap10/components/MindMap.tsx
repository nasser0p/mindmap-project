  ...
  useImperativeHandle(ref, () => ({
    zoomIn: () => {
        if (zoomBehaviorRef.current && svgRef.current) {
            ...
            // FIX: Pass the transition object directly to the zoom behavior method for better type safety and compatibility.
    ...
    const selectionDrag = drag<SVGSVGElement, unknown>()
      ...
      .on('start', (event) => {
        ...
        if (isTouchEvent) {
          ...
          // FIX: The `d3.pointer` function expects a DOM event. The `event` object in a d3-drag handler is a special object; `event.sourceEvent` holds the original DOM event.
          ...
          const currentTransform = zoomTransform(svgRef.current!);
          // FIX: The `d3.pointer` function expects a DOM event. The `event` object in a d3-drag handler is a special object; `event.sourceEvent` holds the original DOM event.
          const [screenX, screenY] = pointer(event.sourceEvent, svgRef.current!);
