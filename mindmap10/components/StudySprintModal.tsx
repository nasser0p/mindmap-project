...
const ConfigView: React.FC<{ onStart: (duration: number) => void }> = ({ onStart }) => {
    ...
    return (
        <div className="w-full max-w-lg mx-auto bg-white rounded-xl shadow-2xl p-8">
            <h2 className="text-2xl font-bold text-slate-800 text-center">Plan Your Study Sprint</h2>
            <p className="text-slate-500 text-center mt-2 mb-6">Let the AI create a focused, timed study session to improve your weakest areas.</p>
            
            <div className="mb-6">
                <label className="block text-sm font-medium text-slate-700 mb-2">How much time do you have?</label>
                <div className="grid grid-cols-3 gap-3">
                    {[15, 30, 45].map((time) => (
                        <button key={time} onClick={() => setDuration(time)} className={`p-4 rounded-lg border-2 transition-all ${duration === time ? 'bg-blue-50 border-blue-500' : 'bg-white hover:bg-slate-50 border-slate-200'}`}>
                            <span className="font-semibold text-slate-800 text-lg">{time}</span>
                            <span className="block text-xs text-slate-500">Minutes</span>
                        </button>
                    ))}
                </div>
            </div>
...
const ActiveSprintView: React.FC<{ sprint: StudySprint, onComplete: () => void }> = ({ sprint, onComplete }) => {
    ...
    return (
         <div className="w-full max-w-3xl h-full flex flex-col bg-white rounded-xl shadow-2xl p-6 md:p-8 overflow-hidden">
            {/* Header and Progress */}
            <div className="mb-4 flex-shrink-0">
                <div className="flex justify-between items-center mb-2">
                    <p className="text-sm font-semibold text-blue-600">Step {currentStepIndex + 1} of {sprint.steps.length}</p>
                    <p className="text-sm font-semibold text-slate-500"><i className="fa-regular fa-clock mr-1.5"></i>{currentStep.duration} min</p>
                </div>
                <div className="w-full bg-slate-200 rounded-full h-2.5">
                    ...
                    {sprint.steps.map((step, index) => (
                        <motion.div
                            key={index}
                            className={`h-full rounded-full ${index <= currentStepIndex ? 'bg-blue-600' : 'bg-slate-200'}`}
            ...
            <div className="flex-1 overflow-y-auto py-4 -mx-4 px-4">
                 <AnimatePresence mode="wait">
                    ...
                    >
                        ...
                        <p className="text-slate-600 whitespace-pre-wrap">{currentStep.instructions}</p>
            ...
            {/* Navigation */}
            <div className="mt-auto pt-6 border-t border-slate-200 flex justify-between items-center flex-shrink-0">
                 <button onClick={() => setCurrentStepIndex(i => i - 1)} disabled={currentStepIndex === 0} className="px-6 py-2 bg-slate-200 text-slate-700 font-semibold rounded-lg hover:bg-slate-300 disabled:opacity-50 disabled:cursor-not-allowed">
                    Previous
                </button>
                {isLastStep ? (
...
const CompletedView: React.FC<{ onClose: () => void }> = ({ onClose }) => (
    <div className="w-full max-w-lg mx-auto bg-white rounded-xl shadow-2xl p-8 text-center">
        <p className="text-6xl mb-4">🎉</p>
        <h2 className="text-2xl font-bold text-slate-800">Study Sprint Complete!</h2>
        <p className="text-slate-500 mt-2 mb-6">Great work! You've taken a big step in mastering this subject. Keep up the momentum!</p>
