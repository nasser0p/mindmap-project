...
interface NodeContextMenuProps {
    ...
    onSetNodeColor: (color: string) => void;
    onFocusNode: () => void;
    onTestBranch: () => void;
...
const NodeContextMenu: React.FC<NodeContextMenuProps> = (props) => {
    const {
        ...
        onExtractConcepts, onGenerateAnalogy, onIdentifyAndLabel, onSetNodeColor, onFocusNode, onTestBranch,
    ...
    return (
        <motion.div
            ref={menuRef}
            className="fixed z-50 w-64 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 p-2 node-context-menu"
            style={style}
        ...
        >
            <button
                onClick={onClose}
                className="absolute top-2 right-2 w-7 h-7 rounded-full flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-slate-200/80 dark:hover:bg-slate-700/80 transition-colors"
                title="Close"
            ...
            {/* FIX: Corrected the loading state for "Identify & Label" to use `isIdentifyingLabels` instead of the incorrect `isGeneratingIdeas`. */}
            <MenuItem icon="fa-tags" text="Identify & Label" onClick={onIdentifyAndLabel} loading={isIdentifyingLabels} disabled={!hasImage} />
            ...
            <ColorPalette onSetColor={onSetNodeColor} />
