...
import { MindMapDocument, MindMapNode, Feedback } from '../types';
...
const useAdminData = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribeDocs = db.collection('documents').onSnapshot(snapshot => {
            const docs = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as MindMapDocument));
            setDocuments(docs);

            // Process stats (this part remains the same)
            ...
            docs.forEach(doc => {
                ...
                if (doc.createdAt > user.lastActive) user.lastActive = doc.createdAt;
                if (!user.tookExam && hasMasteryScore(doc.root)) user.tookExam = true;
                if (doc.sourceDocuments && doc.sourceDocuments.length > 0) aiMapCount++;
            ...
            const stats: UserStat[] = Array.from(userMap.entries()).map(([uid, data]) => ({ uid, ...data }));
            const usersWhoTookExam = Array.from(userMap.values()).filter(u => u.tookExam).length;
            const subjectsPerDay = Array.from(subjectsPerDayMap.entries()).map(([date, count]) => ({ label: new Date(date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), value: count })).reverse();
            const popularTopics = Object.entries(topicCounts).map(([name, count]) => ({ name, count })).sort((a, b) => b.count - a.count).slice(0, 20);
        ...
        const unsubscribeFeedback = db.collection('feedback').orderBy('timestamp', 'desc').onSnapshot(snapshot => {
        ...
        // Set loading to false after both initial fetches attempt to run
        Promise.all([
    ...
    return { documents, feedback, userStats, globalStats, loading, error };
};
